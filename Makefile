...
compile:
	...
	@GOOS=windows GOARCH=386 $(GO) build -v -o bin/$(EXEC_NAME)_win_x32.exe ./cmd/afterburner-exporter

	@echo ">>> compiling for windows x64"
	@GOOS=windows GOARCH=amd64 $(GO) build -v -o bin/$(EXEC_NAME)_win_x64.exe ./cmd/afterburner-exporter

	@echo ">>> compiling for linux x32"
	@GOOS=linux GOARCH=386 $(GO) build -v -o bin/$(EXEC_NAME)_unix_x32 ./cmd/afterburner-exporter

	@echo ">>> compiling for linux x64"
	@GOOS=linux GOARCH=amd64 $(GO) build -v -o bin/$(EXEC_NAME)_unix_x64 ./cmd/afterburner-exporter

	@echo ">>> compiling for arm"
	@GOOS=linux GOARCH=arm $(GO) build -v -o bin/$(EXEC_NAME)_unix_arm ./cmd/afterburner-exporter

	@echo ">>> compiling for arm 64"
	@GOOS=linux GOARCH=arm64 $(GO) build -v -o bin/$(EXEC_NAME)_unix_arm64 ./cmd/afterburner-exporter
